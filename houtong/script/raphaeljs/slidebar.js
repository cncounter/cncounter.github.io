...
(function(Raphael) {
	...
	//
	// 函数
	Raphael.slidebar = function(param) {
		// 初始默认值
		// 采用继承Mixin机制
		param = param || {};
		param = _extends(param, defaultParam);
		//
		return new SlideBar(param);
	};
	// 构造函数
	function SlideBar(param) {
		//
		this._extends(param);
		//
		this.init();
	...
	// 水平
	SlideBar.prototype.processVertical = function (){
		// 用在匿名函数内
		var that = this;
		//
		drawRect(that);
		...
		this.miny = Math.round(this.y);
		this.maxy = Math.round(this.y + this.size - this.csize);
		// 设置颜色
	...
	//
	SlideBar.prototype.getRaphael = function (){
		// 先不处理
		if(!this.raphael){
			//
			if(this.element){
	...
	// 水平
	SlideBar.prototype.processHorizontal = function (){
		// 先不处理
		this.processVertical();
	};
	// 移动处理
	SlideBar.prototype.docOnMove = function(dx, dy, _x, _y) {
		if (this.bOnTheMove) {
			//
			var dd = _x - this.x;
			if(this.vertical){
	...
	// 设置 Bar条
	SlideBar.prototype.setB = function(_dd) {
		// 修正
	...
	SlideBar.prototype.getRatio = function(_dd) {
		if(!this.vertical){
	...
	// 获取/设置颜色
	SlideBar.prototype.setColor = function(_color) {
		//
		this.backrect.attr({
			fill : _color
		});
	...
	SlideBar.prototype.refreshCursor = function() {
		//
		// 修正 value
	...
	SlideBar.prototype.val = function(value, quiet) {
		if (ZERO === arguments.length) {//
			// 不传入参数, 则get返回当前值
			return this.value;
		}
		// 判断是数字 ....判断最大最小值
	...
	SlideBar.prototype.add = function(value, quiet) {
		//
		value = value || ONE;
		var v = this.val();
		return this.val(v + value);
	};
	// 增加 ; (value值, quiet安静模式不触发事件)
	SlideBar.prototype.up = function(quiet) {
		return this.add(ONE,quiet);
	};
	// 减小; (quiet安静模式不触发事件)
	SlideBar.prototype.down = function(quiet) {
		return this.add(MINUS_ONE,quiet);
	};
	//
	SlideBar.prototype._extends = _extends;
	// 继承. 工具方法
