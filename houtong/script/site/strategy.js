...
(function() {
	...
	function fitPaperSize(paper, tree_with_xy, expand_level){
		...
		if(global.config.paper_width < 1 || global.config.paper_height < 1){
			global.config.paper_width = global.config.min_paper_width;
			global.config.paper_height = global.config.min_paper_height;
		}
		var width = global.config.paper_width;
		var height = global.config.paper_height;
		// 比对最小限制
		if(spanX > width){
			width = spanX;
		}
